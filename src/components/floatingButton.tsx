...
export default function FloatingButtonsCustom() {
  ...
  return (
    <div className="fixed bottom-4 z-50 w-full flex justify-center md:bottom-6 md:right-4 md:w-auto lg:bottom-6 lg:right-6">
      <div className="flex md:flex-col items-center gap-2 bg-blue-950 rounded-full px-2 py-2 lg:px-3 lg:py-3 border border-white/10 shadow-md shadow-black/20">
        {buttons.map(({ href, icon: Icon, title }, i) => (
          ...
          >
            ...
            {/* Custom Tooltip */}
            <span className="absolute right-full top-1/2 -translate-y-1/2 mr-3 px-2 py-1 bg-blue-950/90 backdrop-blur-sm text-white text-xs lg:text-sm xl:text-sm rounded-md whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">
              {title}
            </span>
          </a>
        ))}
